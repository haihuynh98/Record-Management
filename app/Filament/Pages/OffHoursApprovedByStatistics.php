...
class OffHoursApprovedByStatistics extends Page implements HasTable
{
    use InteractsWithTable;
    use HasPageShield;

    protected static ?string $navigationIcon = 'heroicon-o-moon';
    protected static ?string $navigationLabel = 'Thống kê người duyệt Ngoài giờ';
    protected static ?string $title = 'Thống kê người duyệt Ngoài giờ';
    protected static ?string $slug = 'off-hours-approved-by-statistics';
    protected static ?string $navigationGroup = 'Thống kê ngoài giờ';
    protected static ?int $navigationSort = 3;

    protected static string $view = 'filament.pages.off-hours-approved-by-statistics';

    public static function getShieldPermissionPrefix(): string
    {
        return 'off_hours_approved_by_statistics';
    }
    ...
    protected function getTableQuery(): Builder
    {
        ...
        return User::query()
            ...
            ->withCount([
                'approvedProfiles as today_count' => function ($query) use ($today, $startTime, $endTime) {
                    $this->applyOffHoursFilter($query, $today, $today, $startTime, $endTime, 'created_at');
                    $query->whereNotIn('status', [2, 3]);
                },
                'approvedProfiles as week_count' => function ($query) use ($weekStart, $startTime, $endTime) {
                    $this->applyOffHoursFilterForRange($query, $weekStart, Carbon::now(), $startTime, $endTime, 'created_at');
                    $query->whereNotIn('status', [2, 3]);
                },
                'approvedProfiles as month_count' => function ($query) use ($monthStart, $startTime, $endTime) {
                    $this->applyOffHoursFilterForRange($query, $monthStart, Carbon::now(), $startTime, $endTime, 'created_at');
                    $query->whereNotIn('status', [2, 3]);
                },
                'approvedProfiles as last_month_count' => function ($query) use ($lastMonthStart, $lastMonthEnd, $startTime, $endTime) {
                    $this->applyOffHoursFilterForRange($query, $lastMonthStart, $lastMonthEnd, $startTime, $endTime, 'created_at');
                    $query->whereNotIn('status', [2, 3]);
                },
                'approvedProfiles as year_count' => function ($query) use ($yearStart, $startTime, $endTime) {
